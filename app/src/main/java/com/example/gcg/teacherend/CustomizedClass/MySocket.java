...
public class MySocket {
    ...
    private String filePathInDevice;//如果是设备从服务器下载的，这个字段保存要写入的存储器地址
    ...
    private byte[] filePath;//文件要存放在服务器的绝对路径
    private byte[] fileContent;//文件内容

    //当要从服务器下载的时候，只需要传入文件在服务器的绝对路径，以及一个回传的handle
    public MySocket(String filePathInDevice,byte[] filePath,Handler handler){
    ...
    public void ReceiveFile(){
        ...
        * */
        ...
        try {
            File file = new File(filePathInDevice);//判断是否存在同名文件
            if(file.exists()){//如果存在就删除
                file.delete();
            }

            fileOutputStream = new FileOutputStream(filePathInDevice);//打开文件流
            ...
            inReceive = clientSocket.getInputStream();
            dataInputStream = new DataInputStream(inReceive);
            bufferedInputStream = new BufferedInputStream(inReceive);

            bufferedOutputStream.write(filePath);//向服务器传送文件存储路径
            bufferedOutputStream.flush();//立即发送
            /*
            int acknowledge = dataInputStream.readInt();//等待服务器回复
            if(acknowledge == 0){
                ReturnMessage("文件名传输失败");
                return;
            }
            */
            ...
            int len;
            do {
                len = bufferedInputStream.read(buffer,0,buffer.length);
                fileOutputStream.write(buffer,0,len);//写入文件
            }while (len != -1 && bufferedInputStream.available() != 0);
            fileOutputStream.flush();
            ReturnMessage("文件下载成功!");
            ...
            //ReturnMessage("文件已下载!");//为什么这个消息可以放出去，但是外面try里面的却不行？
        }
    }
}
